<?php
class M_sudo extends CI_Model
{

  function esc($str){
   return $this->db->escape($str);
  }

####################################################################################################################
####################################################################################################################
####################################################################################################################
  function get_user_details()
   {
   	$sql="SELECT * FROM user_type";
  	return $this->db->query($sql);
   }
  
  function get_nik($user)
   {
   	$user=$this->esc($user);
   	$sql="SELECT NIK FROM teachers where username = $user";
  	return $this->db->query($sql);
   }  
   
  function get_student_info($NIM)
  {
	$NIM=$this->esc($NIM);	
	$sql="SELECT * FROM students WHERE NIM=$NIM LIMIT 1";
	return $this->db->query($sql);
  }
  
  function update_message_state($user,$to){
  	$user=$this->esc($user);
  	$to=$this->esc($to);
  	$where="user_from=$to AND user_to=$user ";
  	$sql="UPDATE user_chats SET state_to=1 WHERE $where";
  	$this->db->query($sql);
  }
   
  function get_teacher_info($NIK)
  {
	$NIK=$this->esc($NIK);
	$sql="SELECT * FROM teachers WHERE NIK=$NIK LIMIT 1";
	return $this->db->query($sql);
  }
  
  function insert_new_posting($user,$category,$title,$content){
  	$user=$this->esc($user);
  	$category=$this->esc($category);
  	$title=$this->esc($title);
  	$content=$this->esc($content);
  	$sql="INSERT INTO posting_pages (`date_time`,`post_title`,`post_content`,`post_categories`,`created_by`,`activated`) 
  		VALUES (NOW(),$title,$content,$category,$user,1)";
  	$this->db->query($sql);
  }
  
  function get_typeOfPost($type){
  	$type=$this->esc($type);
  	$sql="SELECT categories_name FROM post_categories WHERE categories_code=$type LIMIT 1";
  	return $this->db->query($sql);
  }
  
  function select_postByID($id){
  	$id=$this->esc($id);
  	$sql="SELECT * FROM posting_pages WHERE ID_post = $id LIMIT 1";
  	return $this->db->query($sql);
  }
     
  function get_all_user($clue,$user)
  {
  	$c=$this->esc('%'.$clue.'%');
  	$user=$this->esc($user);
  	$where="students.student_name LIKE $c AND `users_account`.username != $user OR teachers.teacher_name LIKE $c AND `users_account`.username != $user";
  	$sql="SELECT t.username, t.sn,t.tn,t.type FROM (SELECT `users_account`.username,LEFT(`users_account`.redirect_path,LOCATE('/',`users_account`.redirect_path)-1) as type,
  		`students`.student_name as sn , `teachers`.teacher_name as tn FROM `users_account` 
		 LEFT JOIN `students` ON `students`.username=`users_account`.username 
		 LEFT JOIN `teachers` ON `teachers`.username=`users_account`.username  WHERE $where) t";
	$q1=$this->db->query($sql);
	foreach($q1->result() as $r){
		$s=$r->sn;
		$t=$r->tn;
		if($s==NULL)$s=$t;
		$data[]=['name'=>$s.' :: '.$r->type,'id'=>$r->username];
	}
	return $data;
  }
   
  function update_teacher_info($NIK,$nama,$email,$gender,$teach,$alamat,$kota,$post,$phone,$info)
   {
     $NIK=$this->esc($NIK);
     $nama=$this->esc($nama);
     $email=$this->esc($email);
     $gender=$this->esc($gender);
     $teach=$this->esc($teach);
     $alamat=$this->esc($alamat);
     $kota=$this->esc($kota);
     $post=$this->esc($post);
     $phone=$this->esc($phone);
     $info=$this->esc($info);
     $sql="UPDATE teachers SET teacher_name=$nama, teaching=$teach, teacher_gender=$gender, 
     teacher_address=$alamat, teacher_city=$kota, teacher_postal_code=$post,
     teacher_phone_number=$phone, teacher_email=$email, teacher_info=$info
     WHERE NIK=$NIK LIMIT 1";
     $this->db->query($sql);
   }
   
  function get_message_count($user)
   {
     $user=$this->esc($user);
     $sql="SELECT ID_chat FROM user_chats WHERE user_to= $user and state_to=0";
     $query=$this->db->query($sql);
     return $query->num_rows();
   }
   
  function insert_new_message_send($user,$userto,$content)
  {
  	$user=$this->esc($user);
  	$userto=$this->esc($userto);
  	$content=$this->esc($content);
  	$sql="INSERT INTO user_chats (`date_time`,`user_from`,`user_to`,`chat_content`,`state_from`) VALUES (NOW(),$user,$userto,$content,1)";
  	$this->db->query($sql);
  }
  
  function select_user_fromUsername($username){
  	$user=$this->esc($username);
  	$sql="SELECT username FROM users_account WHERE username=$user LIMIT 1";
  	return $this->db->query($sql);
  }
  
  function select_chats_fromCurrentuser($user,$me)
  {
    $from=$this->esc($user);
    $user=$this->esc($me);
    $where ="user_from = $user and user_to= $from OR user_from = $from and user_to=$user";
    $sql="SELECT * FROM user_chats WHERE $where ORDER BY date_time DESC";
    return $this->db->query($sql);
  }
  
  function sendReport($user,$content,$subject){
  	$user=$this->esc($user);
  	$content=$this->esc($content);
  	$subject=$this->esc($subject);
  	$sql="INSERT INTO report_problem (`date`,`from`,`subject`,`content`) VALUES (NOW(),$user,$subject,$content)";
  	$this->db->query($sql);
  }
   
  function get_messages($user)
   {
     $user=$this->esc($user);
     $sql="SELECT * FROM (SELECT * FROM user_chats WHERE user_to= $user ORDER BY state_to ASC) t1 GROUP BY user_from";
     $query=$this->db->query($sql);
     return $query;
   }
   
  function get_outmessages($user)
   {
     $user=$this->esc($user);
     $sql="SELECT * FROM user_chats WHERE user_from= $user ORDER BY state_to ASC";
     $query=$this->db->query($sql);
     return $query;
   }
   
  function get_student_score_where($kelas,$subject,$nim)
   {
   	$where_kelas='';$where_subject='';$where_nim='';
   	if ($kelas != ""){
   	  $kelas=$this->esc($kelas);
   	  $where_kelas="AND student_test_report.class = $kelas";}
   	if ($subject != "" and $subject != "none"){
   	  $subject=$this->esc($subject);
   	  $where_subject="AND student_test_report.subject_code=$subject";}
   	if ($nim != "") {$nim=$this->esc($nim); 	  $where_nim="AND student_test_report.NIM=$nim";}
   	 $sql="SELECT student_test_report.*,subject.subject_name FROM student_test_report 
   		LEFT JOIN subject ON student_test_report.subject_code=subject.subject_code 
   			WHERE  student_test_report.activated=1 $where_kelas $where_subject $where_nim";
   	return $this->db->query($sql);	
   }
   
  function execute_action($act,$e){
  	/* 1="setujui/publish"
  	*  2="hapus"
  	*/
  	$ID=$this->esc($e);
  	if ($act==1){
	  	$sql="UPDATE student_test_report SET activated=1 where ID_test=$ID AND activated=0 LIMIT 1";
  	}elseif($act==2){
  		$sql="DELETE FROM student_test_report WHERE ID_test=$ID AND activated = 0 LIMIT 1";
  	}else{
  		$sql="";
  	}
  	$this->db->query($sql);
  }
   
  function get_student_score_pending($kelas)
   {
   	$kelas=$this->esc($kelas);
   	 $sql="SELECT student_test_report.*,subject.subject_name FROM student_test_report 
   		LEFT JOIN subject ON student_test_report.subject_code=subject.subject_code 
   			WHERE student_test_report.class = $kelas AND student_test_report.activated = 0";
   	return $this->db->query($sql);	
   }
   
  function get_all_active_class()
   {
     $sql="SELECT * FROM classes WHERE activated = 1 ORDER BY class_code ASC";
     return $this->db->query($sql);
   }
   
  function get_all_active_subject()
   {
     $sql="SELECT * FROM subject WHERE activated = 1 ORDER BY subject_code ASC";
     return $this->db->query($sql);
   }
   
   function select_fromMessageID($id){
     $id=$this->esc($id);
     $sql="SELECT * FROM user_chats WHERE ID_chat=$id LIMIT 1";
     return $this->db->query($sql);
   }
   
   function get_student_scoreByNIM($nim){
     $nim=$this->esc($nim);
     $sql="SELECT student_test_report.*,subject.subject_name FROM student_test_report 
     LEFT JOIN subject ON  subject.subject_code=student_test_report.subject_code WHERE student_test_report.NIM=$nim ";
     return $this->db->query($sql);
   }
   
  function insert_student_score($user,$nim,$kelas,$subject,$u1,$u2,$u3,$u4,$u5,$u6,$u7,$u8,$u9,$u10,$user){
  	//$user=$this->esc($user);
  	$nim=$this->esc($nim);
  	$kelas=$this->esc($kelas);
  	$subject=$this->esc($subject);
  	$u1=$this->esc($u1);
  	$u2=$this->esc($u2);
  	$u3=$this->esc($u3);
  	$u4=$this->esc($u4);
  	$u5=$this->esc($u5);
  	$u6=$this->esc($u6);
  	$u7=$this->esc($u7);
  	$u8=$this->esc($u8);
  	$u9=$this->esc($u9);
  	$u10=$this->esc($u10);
  	$user=$this->esc($user);
  	$sql="INSERT INTO student_test_report(`user`,`NIM`,`class`,`subject_code`,`u1`,`u2`,`u3`,`u4`,`u5`,`u6`,`u7`,`u8`,`u9`,`u10`,`activated`) 
  	VALUES ($user,$nim,$kelas,$subject,$u1,$u2,$u3,$u4,$u5,$u6,$u7,$u8,$u9,$u10,1)";
  	$this->db->query($sql);
  }
  
  function update_post($id,$categories,$title,$content,$state){
  	$id=$this->esc($id);
  	$cat=$this->esc($categories);
  	$title=$this->esc($title);
  	$content=$this->esc($content);
  	$sql="UPDATE posting_pages SET `date_time`=NOW() ,`post_title`=$title,`post_content`=$content,`post_categories`=$cat ,`activated`=$state
  	WHERE ID_post=$id";
  	$this->db->query($sql);
  }
  
  
  function delete_student($id){
  	$id=$this->esc($id);
  	$sql="DELETE from students WHERE NIM=$id LIMIT 1";
  	$this->db->query($sql);
  }
  
  function delete_post($id){
  	$id=$this->esc($id);
  	$sql="DELETE from posting_pages WHERE ID_post=$id LIMIT 1";
  	$this->db->query($sql);
  }
  
  function get_specific_complaint($id){
  	$id=$this->esc($id);
	$sql="SELECT * FROM report_problem WHERE ID_report= $id LIMIT 1";
  	return $this->db->query($sql);
  }
  
  function update_post_state($id,$action){
  	if($action == 1 and $id != ""){
  	 foreach($id as $r){
  	  $this->db->query("UPDATE posting_pages SET activated=1 WHERE ID_post=$r");
  	 }
  	}elseif($action == 0 and $id != ""){
  	 foreach($id as $r){
  	  $this->db->query("DELETE FROM posting_pages WHERE ID_post = $r");
  	 }
  	}
  }
  
  function save_detail_student($nim,$nama,$kelas,$jurusan,$email,$agama,
	  	$jk,$alamat,$kota,$kode_pos,$notelp,$info,$nama_ortu,$jk_ortu,$agama_ortu,
	  	$alamat_ortu,$kota_ortu,$kodepost_ortu,$notelp_ortu,$state)
  {
  	$nim=$this->esc($nim);			$nama=$this->esc($nama);		$kelas=$this->esc($kelas);
  	$jurusan=$this->esc($jurusan);		$email=$this->esc($email);	$agama=$this->esc($agama);
  	$jk=$this->esc($jk);			$alamat=$this->esc($alamat);	$kota=$this->esc($kota);	
  	$kode_pos=$this->esc($kode_pos);		$notelp=$this->esc($notelp);	$info=$this->esc($info);
  	$nama_ortu=$this->esc($nama_ortu);	$jk_ortu=$this->esc($jk_ortu);	$agama_ortu=$this->esc($agama_ortu);	
  	$alamat_ortu=$this->esc($alamat_ortu);  	$kota_ortu=$this->esc($kota_ortu);	$kodepost_ortu=$this->esc($kodepost_ortu);
  	$notelp_ortu=$this->esc($notelp_ortu);	$state=$this->esc($state);
  	 $sql="UPDATE students SET student_name=$nama,	student_class=$kelas,	student_major=$jurusan,	student_email=$email
  		,religion=$agama, student_gender=$jk,	student_address=$alamat,student_city=$kota,	student_postal_code=$kode_pos,
  		student_phone_number=$notelp,		student_info=$info,	parent_name=$nama_ortu,	parent_gender=$jk_ortu,
  		parent_religion=$agama_ortu,		parent_address=$alamat_ortu,parent_city=$kota_ortu, parent_post=$kodepost_ortu,parent_number=$notelp_ortu,
  		activated=$state WHERE NIM=$nim";
  	$this->db->query($sql);
  }
  
  function update_score($type,$id,$kelas,$subject,$u1,$u2,$u3,$u4,$u5,$u6,$u7,$u8,$u9,$u10,$m1,$s1,$m2,$s2)
  {
  	$id=$this->esc($id);
  	$kelas=$this->esc($kelas);
  	$subject=$this->esc($subject);
  	if($type=="ulangan_harian"){
	  	$u1=$this->esc($u1);
	  	$u2=$this->esc($u2);
	  	$u3=$this->esc($u3);
	  	$u4=$this->esc($u4);
	  	$u5=$this->esc($u5);
	  	$u6=$this->esc($u6);
	  	$u7=$this->esc($u7);
	  	$u8=$this->esc($u8);
	  	$u9=$this->esc($u9);
	  	$u10=$this->esc($u10);
		$sql="UPDATE student_test_report SET `subject_code`=$subject,`u1`=$u1,`u2`=$u2,`u3`=$u3,`u4`=$u4,`u5`=$u5,`u6`=$u6,`u7`=$u7,`u8`=$u8,`u9`=$u9,`u10`= $u10
			WHERE `ID_test`=$id AND `class`=$kelas LIMIT 1";	
  	}elseif($type=="ulangan_smtr"){
	  	$m1=$this->esc($m1);
	  	$m2=$this->esc($m2);
	  	$s1=$this->esc($s1);
	  	$s2=$this->esc($s2);
	  	$sql="UPDATE student_test_report SET `mid_semester1`=$m1,`mid_semester2`=$m2,`semester1`=$s1, `semester2`=$s2
			WHERE ID_test=$id and class=$kelas ";  
  	}
  	if(!empty($sql))$this->db->query($sql);
  }
  
  function delete_score_where_id($id)
  {
  	$id=$this->esc($id);
  	$sql="DELETE FROM student_test_report WHERE ID_test=$id LIMIT 1";
  	$this->db->query($sql);
  }
  
  function get_chat_from_id($id){
  	$id=$this->esc($id);
  	$sql="SELECT user_to,user_from FROM user_chats WHERE ID_chat=$id";
  	return $this->db->query($sql);
  }
  
  function delete_messages_from_user($user,$user2){
  	$user=$this->esc($user);
  	$user2=$this->esc($user2);
  	$where="user_to=$user AND user_from=$user2 OR user_to=$user2 AND user_from=$user ";
  	$sql="DELETE FROM user_chats WHERE $where";
  	$this->db->query($sql);
  }
  
  function update_smtr($state,$nim,$user,$kls,$m1,$m2,$s1,$s2)
  {
  	$nim=$this->esc($nim);
  	$user=$this->esc($user);
  	$kls=$this->esc($kls);
  	$m1=$this->esc($m1);
  	$m2=$this->esc($m2);
  	$s1=$this->esc($s1);
  	$s2=$this->esc($s2);
  	if($state==1){
		$sql="UPDATE student_test_report SET `mid_semester1`=$m1,`mid_semester2`=$m2,`semester1`=$s1, `semester2`=$s2
				WHERE nim=$nim and user=$user and class=$kls ";  		
  		}else{
  			$sql="INSERT INTO student_test_report (`NIM`,`user`,`class`,`mid_semester1`,`mid_semester2`,`semester1`, `semester2`) 
  			VALUES($nim,$user,$kls,$m1,$m2,$s1,$s2)";
  			}
  		$this->db->query($sql);
  	}
   
  function get_nim_nama()
   {
   	$sql="SELECT NIM,student_name FROM students where 1";
  	return $this->db->query($sql);
   }
   
  function get_duplicate_score($nim,$kelas,$subject)
   {
   	$nim=$this->esc($nim);
   	$kelas=$this->esc($kelas);
   	$subject=$this->esc($subject);
   	$sql="SELECT NIM FROM student_test_report WHERE NIM=$nim AND class=$kelas AND subject_code=$subject LIMIT 1";
   	$q=$this->db->query($sql);
   	return $q->num_rows();
   }
  
  function get_scores($nim)
   {
   	$NIM=$this->esc($nim);
   	$sql="SELECT student_test_report.*, students.student_name, subject.subject_name FROM student_test_report LEFT JOIN students 
   		ON students.NIM = student_test_report.NIM LEFT JOIN subject ON subject.subject_code = student_test_report.subject_code
   		where student_test_report.NIM = $NIM";
  	return $this->db->query($sql);
   }
  
  function get_subject_name($value)
   {
   	$value=$this->esc($value);
   	$sql="SELECT subject_name FROM subject WHERE subject_code=$value LIMIT 1";
   	return $this->db->query($sql);
   }
   
  function get_subject()
   {
     $sql="SELECT subject_code,subject_name FROM subject WHERE activated=1";
     $query=$this->db->query($sql);
     return $query;
   }
  
  function get_siswa_information($user)
   {
   	$user=$this->esc($user);
   	$sql="SELECT * FROM students where username= $user LIMIT 1";
  	return $this->db->query($sql);
   }
   
  function get_all_categories()
   {
     $sql="SELECT categories_code,categories_name FROM post_categories WHERE 1";
     return $this->db->query($sql);
   }
  
  function get_total_notifications()
   {
  	$sql="SELECT ID_post as num FROM posting_pages where post_categories='notif'";
  	$q=$this->db->query($sql);
  	
	return $q->num_rows();
   }
  
  function get_notifications($awal=0,$akhir)
   {
  	$sql="SELECT * FROM posting_pages where post_categories='notif' order by date_time DESC LIMIT $awal,$akhir";
  	return $this->db->query($sql);
   }
   
  function get_all_students_where($class,$jur='')
  {
  	$class=$this->esc($class);
  	$jur=$this->esc($jur);
  	if($jur == '')$whr_jur="";else $whr_jur="and student_major=$jur";
  	$sql="SELECT NIM,student_name,student_class,student_major,student_email,parent_name,student_phone_number,activated FROM students WHERE student_class = $class $whr_jur  LIMIT 30";
  	return $this->db->query($sql);
  }
   
  function get_all_students()
  {
  	$sql="SELECT NIM,student_name,student_photo,student_major FROM students WHERE student_class = $class $whr_jur and activated = 1 LIMIT 30";
  	return $this->db->query($sql);
  }
   
  function get_all_teacher_where($teach)
  {
  	$teach=$this->esc($teach);
  	$sql="SELECT NIK,teacher_name,teacher_photo FROM teachers WHERE teaching=$teach AND activated = 1 LIMIT 30";
  	return $this->db->query($sql);
  }
   
  function get_all_teacher()
  {
  	
  	$sql="SELECT * FROM teachers  WHERE activated = 1";
  	return $this->db->query($sql);
  }
   
  function get_notification_whereID($id)
   {
   $id=$this->esc($id);
   $sql="SELECT * FROM posting_pages where ID_post=$id";
  	return $this->db->query($sql);	
   }
   
  function get_report_where($userTo)//userto adalah nama user dari tujuan complaint
   {
   $user=$this->esc($userTo);
   $sql="SELECT * FROM report_problem where 1 ORDER BY state ASC,date DESC";
  	return $this->db->query($sql);	
   }
   
   function delete_complaint($id){
    $id=$this->esc($id);
    $sql="DELETE FROM report_problem WHERE ID_report = $id";
    $this->db->query($sql);
   }
   
   function update_compaint_state($id){
    $id=$this->esc($id);
    $sql="UPDATE report_problem SET state=1 WHERE ID_report=$id LIMIT 1";
    $this->db->query($sql);
   }
   
  function get_content_detail($id)
   {
   	$id=$this->esc($id);
   	$sql="SELECT * FROM posting_pages where ID_post = $id and activated = 1 LIMIT 1";
   	return $this->db->query($sql);
   }
   
  function get_class_name($val)
   {
   	$val=$this->esc($val);
   	$sql="SELECT class_name FROM classes where class_code = $val LIMIT 1";
   	return $this->db->query($sql);
   }
   
  function get_detail_siswa($nim){
  	$nim=$this->esc($nim);
  	$sql="SELECT * FROM students WHERE NIM=$nim LIMIT 1";
  	return $this->db->query($sql);
  }
   
  function get_teachers()
   {
   	$sql="SELECT teachers.NIK,teachers.teacher_name,teachers.teacher_photo,teachers.teaching, teacher_details.teacher_phone_number, subject.subject_name FROM teachers 
   		LEFT JOIN teacher_details ON teachers.NIK=teacher_details.NIK LEFT JOIN subject ON subject.subject_code=teachers.teaching";
   	return $this->db->query($sql);
   }
   
  function get_teaching($user)
   {
  		$object='';
  		$user=$this->esc($user);
  		$sql="SELECT teachers.teaching,subject.subject_name FROM teachers LEFT JOIN subject ON subject.subject_code=teachers.teaching 
  				WHERE username=$user LIMIT 1";
  		$hasil=$this->db->query($sql);
  		$array="";
  		foreach($hasil->result() as $row){
  			  $array=array($row->subject_name,$row->teaching);
  			}
  		return $array;
  	}
   
  function get_friends($user)
   {
   	$user=$this->esc($user);
   	$sql="SELECT * FROM students where username != $user and activated = 1";
   	return $this->db->query($sql);
   }
   
  function get_left_content($user)
   {
   	$user=$this->esc($user);
   	$sql="SELECT students.NIM, students.student_name, students.student_class, student_photo FROM students 
   	LEFT JOIN student_details ON students.NIM=student_details.NIM where username = $user ";
   	return $this->db->query($sql);
   }
   
  function get_my_profile($nik)
   {
     $NIK=$this->esc($nik);
     $sql="SELECT * FROM teachers where NIK = $NIK LIMIT 1";
     return $this->db->query($sql);
   }
   
  function get_data_before_trash($tb,$col,$val){
  	$val=$this->esc($val);
  	$sql="SELECT * FROM $tb WHERE $col=$val";
  	$q=$this->db->query($sql);
  	return $q->result();
  }
   
  function trash($tb,$content){
  	$tb=$this->esc($tb);
  	$content=$this->esc($content);
  	$sql="INSERT INTO trash(`tgl`,`table`,`content`)
  		VALUES(NOW(),$tb,$content)";
  	$this->db->query($sql);
  }
   
  function get_major($maj)
   {
   	$major=$this->esc($maj);
   	$sql="SELECT major_name FROM majors where major_code = $major ";
   	return $this->db->query($sql)->result();
   }
   
  function check_if_exists_score($id,$NIM){
  	$id=$this->esc($id);
  	$NIM=$this->esc($NIM);
  	$sql="SELECT * FROM student_test_report WHERE ID_test=$id AND NIM=$NIM LIMIT 1";
  	$query=$this->db->query($sql);
  	return $query->num_rows();
  }
  
  function get_student_score_by_id($id){
  	$id=$this->esc($id);
  	$sql="SELECT * FROM student_test_report WHERE ID_test=$id LIMIT 1";
  	return $this->db->query($sql);
  }
   
  function get_all_major()
   {
   	$sql="SELECT major_code,major_name FROM majors where activated=1 ";
   	return $this->db->query($sql);
   }
   
  function get_username_from_NIK($code)
   {
   	$nik=$this->esc($code);
   	$sql="SELECT username FROM teachers where NIK = $nik LIMIT 1";
   	$q=$this->db->query($sql)->result();
   	$username='';
   	foreach($q as $r){$username=$r->username;}
   	if ($username != ''){$u=$username;}else{$u='';}
   	return $u;
   }
   
  function get_chats($user,$to)
   {
   	$user=$this->esc($user);
   	$to=$this->esc($to);
   	$sql="SELECT user_chats.*, student_details.student_photo FROM user_chats 
   	LEFT JOIN students ON students.username=user_chats.user_from 
   	LEFT JOIN student_details ON students.NIM=student_details.NIM
   	where user_from = $user and user_to = $to order by date_time DESC";
   	$q=$this->db->query($sql)->result();
   	return $q;
   }
   
  function get_data()
   {
   	$sql="SELECT * FROM students where 1 ";
  	return $this->db->query($sql);
   }
   
  function insert_new_student($nim, $student_name, $class, $major, $student_email, 
				$student_religi, $student_gender, $student_addr, $student_city, 
				$student_post, $student_noTelp, $student_info, $parent_name, 
				$parent_gender, $parent_agama, $parent_addr, $parent_city, 
				$parent_post, $parent_noTelp, $photo, $state)
   {
   	
   
   }

}
//end of file 


